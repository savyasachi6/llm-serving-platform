...
class Orchestrator:
    ...
    async def process_ticket(self, ticket_text: str) -> dict:
        ...
        try:
            ...
            raise RuntimeError(
                f"Pipeline halted: upstream agent failed during processing ({exc})"
            ) from exc
        ...
        redacted_text = (
            redact_result.get("redacted_text") if isinstance(redact_result, dict) else None
        )
        if not redacted_text or not redacted_text.strip():
            logger.critical(
                "PII Redaction failed or returned empty text. Halting pipeline to prevent data leakage."
            )
            raise RuntimeError(
                "Security Violation: RedactAgent failed to produce sanitized text. Pipeline terminated."
            )
        ...
        try:
            respond_result = await self.respond.execute(
                {"classification": classification, "redacted_text": redacted_text}
            )
        except Exception as exc:
