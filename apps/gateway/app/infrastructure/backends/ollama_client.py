        ...
        try:
            ...
            response = await client.post(
                ...
                json=request.model_dump(
                    exclude_none=True,
                    exclude={"workload_type", "priority", "cache_key", "tenant_scope"},
                ),
            )
            ...
            raise HTTPException(
                status_code=status.HTTP_504_GATEWAY_TIMEOUT, detail="Ollama timeout"
            )
        except httpx.HTTPStatusError as e:
            raise HTTPException(
                status_code=status.HTTP_502_BAD_GATEWAY, detail=f"Ollama error: {e.response.text}"
            )
